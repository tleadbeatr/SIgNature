...
const prompt =
    ...
    `First identify the physical expression the participant intended to submit, treat that expression literally: do not add, remove, replace or ignore any quantity or factor from it. Then interpret the complete expression physically. You may simplify the units algebraically, but every factor must survive unless it is explicitly cancelled by a valid unit conversion or mathematical operation. Do not turn a quantity into a familiar physical quantity merely because part of its units resembles one.\n\n` +
    'Explain what the complete expression represents physically. You may simplify the units to discover its physical dimensions and meaning, but every factor in the original expression must remain accounted for. Explain what physical quantity the participant has constructed, including any interesting or surprising consequences of the combination. Do not force it into a familiar category if that does not fit. If the combination is unusual, explain why.\n\n' +
